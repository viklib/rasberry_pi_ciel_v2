...
```
Fonctionnement :
Cliquer sur Activer pour initialiser la broche GPIO de la LED
Régler le seuil de température souhaité
Cliquer sur Demarrer acquisition pour lancer la lecture du capteur toutes les 2 secondes
Si la température dépasse le seuil → la LED s'allume automatiquement
Si la température repasse sous le seuil → la LED s'éteint automatiquement
---
Structure du projet
```
appS5/
